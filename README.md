...
- `TELEGRAM_HASH`: This is to authenticate your Telegram account for downloading Telegram files. You can get this from <https://my.telegram.org>. `Str`
    ...
    - `RSS_SPECIFIC_URL`: Legacy single feed URL that should be treated as "first-only". Kept for backward compatibility. `Str`
    - `RSS_FIRST_ONLY_URLS`: Space or comma separated list of feed URLs where only the very first item should be fetched and auto-leech each cycle. `/rss get` is also limited to the first item for these feeds. `Str`
    ...
    <li><details>
        <summary><b>TamilBlasters Auto Leech</b></summary>

    - `AUTO_TBL_CHANNEL`: Destination chat/channel where new torrents will be posted. `Int | Str`
    - `AUTO_TBL_COMMAND_CHAT`: Chat used to trigger the qBittorrent leech command. Defaults to `AUTO_TBL_CHANNEL` when empty. `Int | Str`
    - `AUTO_TBL_FEEDS`: Comma/space separated list or JSON array of TamilBlasters home URLs to crawl (ex: `https://www.1tamilmv.land/`). `Str`
    - `AUTO_TBL_THUMBNAIL`: Optional image URL to download once and reuse as thumbnail for posted torrents. `Str`
      - **NOTES**: Processed torrents/topics are cached in `auto_tbl_state.json`, so duplicates are not reposted after restarts.
