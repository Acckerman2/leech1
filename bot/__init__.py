...
def _parse_feed_list(raw_value: str, default) -> list:
    if not raw_value:
        return default.copy()
    urls: list[str] = []
    try:
        ...
        urls = [item.strip() for item in raw_value.replace(',', ' ').split() if item.strip()]
    if not urls:
        return default.copy()
    return urls
...
RSS_SPECIFIC_URL = environ.get('RSS_SPECIFIC_URL', '')
if len(RSS_SPECIFIC_URL) == 0:
    RSS_SPECIFIC_URL = ''

RSS_FIRST_ONLY_URLS = environ.get('RSS_FIRST_ONLY_URLS', '')
if len(RSS_FIRST_ONLY_URLS) == 0:
    RSS_FIRST_ONLY_URLS = []
else:
    urls = RSS_FIRST_ONLY_URLS.replace(',', ' ').split()
    RSS_FIRST_ONLY_URLS = list(dict.fromkeys(urls))

DEFAULT_TBL_FEED = "https://www.1tamilmv.land/"
AUTO_TBL_FEEDS_RAW = environ.get('AUTO_TBL_FEEDS', '')
AUTO_TBL_FEEDS = _parse_feed_list(AUTO_TBL_FEEDS_RAW, [DEFAULT_TBL_FEED])
...
AUTO_TBL_THUMBNAIL = environ.get('AUTO_TBL_THUMBNAIL', '')
if len(AUTO_TBL_THUMBNAIL) == 0:
    AUTO_TBL_THUMBNAIL = ''
...
config_dict = {'ANIME_TEMPLATE': ANIME_TEMPLATE,
               ...
               'AUTO_TBL_FEEDS': AUTO_TBL_FEEDS,
               'AUTO_TBL_THUMBNAIL': AUTO_TBL_THUMBNAIL,
               'RSS_CHAT': RSS_CHAT,
               'RSS_DELAY': RSS_DELAY,
               'RSS_SPECIFIC_URL': RSS_SPECIFIC_URL,
               'RSS_FIRST_ONLY_URLS': RSS_FIRST_ONLY_URLS,
               'SAVE_MSG': SAVE_MSG,
               'SAFE_MODE': SAFE_MODE,
