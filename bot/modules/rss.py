...
rss_dict_lock = Lock()
handler_dict = {}
FIRST_ONLY_LINKS = set(config_dict.get('RSS_FIRST_ONLY_URLS') or [])
if config_dict.get('RSS_SPECIFIC_URL'):
    FIRST_ONLY_LINKS.add(config_dict['RSS_SPECIFIC_URL'])
...
async def rssGet(client, message, pre_event):
    ...
    try:
        ...
        if data and count > 0:
            first_only_feed = data['link'] in FIRST_ONLY_LINKS
            if first_only_feed:
                count = 1
            try:
                forced_note = " (restricted to first item)" if first_only_feed else ""
                msg = await sendMessage(message, f"Getting the last <b>{count}</b> item(s){forced_note} from {title}")
                async with ClientSession(trust_env=True) as session:
                ...
                while True:
                    ...
                    try:
                        ...
                        if data['last_feed'] == url or data['last_title'] == item_title:
                            break
                        if first_only_feed and feed_count >= 1:
                            break
                    except IndexError:
                        LOGGER.warning(
